...
from marine_mammals_classes_mapping import CLASSES_MAPPING_REDUCED as classes_mapping
...
def predict(audio_clip, confidence_threshold=0.5, direction=None):
    ...
    return mel_spectrogram_image, processed_prediction, current_segment_audio, segment_info
...
def main():
    with gr.Blocks() as iface:
        with gr.Row():
            ...
            confidence_threshold.change(fn=update_interface, inputs=[audio_clip, confidence_threshold], outputs=[mel_spectrogram, predictions, segment_audio, segment_info])
            audio_clip.change(fn=update_interface, inputs=[audio_clip, confidence_threshold], outputs=[mel_spectrogram, predictions, segment_audio, segment_info])
            left_button.click(fn=navigate_left, inputs=[audio_clip, confidence_threshold], outputs=[mel_spectrogram, predictions, segment_audio, segment_info])
            right_button.click(fn=navigate_right, inputs=[audio_clip, confidence_threshold], outputs=[mel_spectrogram, predictions, segment_audio, segment_info])
